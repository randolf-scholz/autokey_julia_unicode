...
\iftoggle{subscript@continue}{%
	...
	\let\subscript@list=\relax%
	\ifmmode%
		\def\subscript@list{\sb\bgroup}
		\sb{#1\subscript@connect}%
	\else%
		\textsubscript{#1\subscript@connect}%
	\fi%
}%
%%
%#1%
...
\ifmmode%
	\sb\bgroup\subcript@list\egroup%
\else%
	\
...
\ifx#1\subscript%
	\typeout{Keep going!}
	\def\subscript@check@next{#1}
\else
	\typeout{Terminating!}
	\togglefalse{subscript@continue}
	\def\subscript@check@next{\relax}
%	\ifmmode\egroup\else\endgroup\fi
\fi
%
\subscript@check@next%
%
}
